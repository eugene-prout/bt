...
BT::BitTorrentMessage BT::ReadMessage(TCPConnection &connection) {
  ...
  std::transform(messageSize.begin(), messageSize.end(), digits.begin(), [](char c) { return static_cast<int>(c); });
  ...
  if (amountLeftToRead != 0) {
    ...
    std::transform(bodyText.begin(), bodyText.end(), body.begin(), [](unsigned char c) { return std::byte{c}; });
  }
...
BT::Peer::Peer(TCPConnectionFactory &connectionFactory, PeerDetails &details, const std::vector<std::byte> &infoHash)
  ...
  if (!HandshakeWithPeer(connection, infoHash))
    throw std::runtime_error("Failed to handshake with peer. Peer responded "
  ...
  while (true) {
    ...
    if (message.Type == MessageType::Unchoke) {
      ...
      if (downloadedPieces.contains(havePiece)) {
        std::cout << std::format("Have message contains already downloaded "
                                 "piece {}, skipping download.",
                                 havePiece)
                  << std::endl;
        continue;
      } else if (pieceInProgress.has_value()) {
        seenPieces.push_back(havePiece);
        std::cout << std::format("Have message contains an undownloaded piece {}, but "
                                 "a piece download is currently in progress. "
                                 "Adding to seen but not starting download.",
                                 havePiece)
                  << std::endl;
      } else {
        std::cout << std::format("Have message contains an undownloaded piece {}, and "
                                 "no piece download is currently in progress. "
