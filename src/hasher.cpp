...
std::vector<std::byte> Hasher::GetSHA1HashAsBytes(const std::string &source) {
  ...
  CryptoPP::StringSource test{source, true, new CryptoPP::HashFilter(sha1, new CryptoPP::VectorSink(hash))};

  return hash | std::views::transform([](CryptoPP::byte c) { return std::byte(c); }) |
