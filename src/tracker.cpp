...
BT::TrackerResponse BT::QueryTrackerForPeers(TorrentFile &torrentFile) {
  std::map<std::string, std::string> query_parameters = {{"info_hash", UrlEncodeBytes(torrentFile.InfoHash)},
  ...
  auto url_struct =
      Url(torrentFile.AnnounceUrl.protocol, torrentFile.AnnounceUrl.hostname, torrentFile.AnnounceUrl.port, path);
  ...
  if (peers_present) {
    ...
    auto byteGroups = peers_string | std::views::transform([](char c) { return std::byte(c); }) | std::views::chunk(6);
