...
BT::BitTorrentMessage::BitTorrentMessage(MessageType type, int length, std::vector<std::byte> content) : Type(type), Length(length), Content(std::move(content))
{
}
...
BT::BitTorrentMessage BT::MessageFactory::GetInterestedMessage()
{
    return { MessageType::Interested, 1, {} };
}
