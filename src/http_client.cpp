...
BT::HTTPClient::~HTTPClient() { SSL_CTX_free(ctx); }

enum class ParsingState { None, ParsingHeaders, ParsingBody, ParsingBodyChunk };
...
// TODO: add timeouts
BT::HTTPResponse BT::HTTPClient::MakeHTTPGetRequest(Url requestUrl) {
...
// TODO: think about how this handles the SSL/TLS records chunking and
// buffering.
...
BT::HTTPResponse BT::HTTPClient::MakeHTTPSGetRequest(Url requestUrl) {
  ...
  for (ai = res; ai != NULL; ai = BIO_ADDRINFO_next(ai)) {
    // We could equally use non-OpenSSL calls such as "socket" here for this and
    // the subsequent connect and close functions. But for portability reasons
    // and also so that we get errors on the OpenSSL stack in the event of a
    // failure we use OpenSSL's versions of these functions.
  ...
  if (SSL_connect(ssl) < 1) {
    ...
    // If the failure is due to a verification error we can get more information
    // about it from SSL_get_verify_result().
  ...
  // Check whether we finished the while loop above normally or as the result of
  // an error. The 0 argument to SSL_get_error() is the return code we received
  // from the SSL_read_ex() call. It must be 0 in order to get here. Normal
  // completion is indicated by SSL_ERROR_ZERO_RETURN.
