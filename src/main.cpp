...
struct parse_command {
  ...
  parse_command(lyra::cli &cli) {
    cli.add_argument(
        lyra::command("parse", [this](const lyra::group &g) { this->do_command(g); })
            .help("Parses the provided .torrent file.")
            .add_argument(lyra::help(show_help))
            .add_argument(lyra::arg(file_name, "filename").required().help("The file path for the .torrent file.")));
  }
...
struct download_command {
  ...
  download_command(lyra::cli &cli) {
    cli.add_argument(
        lyra::command("dl", [this](const lyra::group &g) { this->do_command(g); })
