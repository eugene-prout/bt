...
std::string BT::UrlEncodeByte(const std::byte byte) { return std::format("%{:02X}", std::to_integer<int>(byte)); }
...
std::vector<std::string> BT::ToHex(const std::span<const std::byte> v) {
  ...
  std::transform(std::begin(v), std::end(v), std::back_inserter(bytes),
                 [](std::byte byte) { return std::format("{:02X}", std::to_integer<int>(byte)); });
...
std::pair<std::string, std::string> BT::SplitOnFirst(const std::string_view sv, const std::string &delimiter) {
...
std::string BT::IPAddressToString(const std::span<const std::byte> ipAddressOctets) {
  return ipAddressOctets | std::views::transform([](std::byte b) { return std::to_string(std::to_integer<int>(b)); }) |
         std::views::join_with('.') | std::ranges::to<std::string>();
}

std::array<std::byte, 4> BT::EncodeIntegerForMessage(const unsigned int integer) {
  return std::array<std::byte, 4>{
      static_cast<std::byte>((integer >> 24) & 0xFF), static_cast<std::byte>((integer >> 16) & 0xFF),
      static_cast<std::byte>((integer >> 8) & 0xFF), static_cast<std::byte>(integer & 0xFF)};
}
